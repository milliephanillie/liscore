...
function OwnerAlt(props) {
  ...
  return (
    <Fragment>
      ...
      <div className="profile--owner profile-owner-updated">
        ...
        {!isEmpty(product?.premium_profile?.website) && options?.display_website &&
        <div className="lisfinity-product--info flex">
              ...
              >
                <ReactSVG
                  src={`${lc_data.dir}dist/${World}`}
                  className="w-14 h-14 fill-grey-1000"
                />
              </span>
          <span
            className="text-lg text-grey-700 mr-6">{lc_data.jst[735]}</span>
          <a href={product?.premium_profile?.website}
             className="text-lg text-grey-1000 break-all"> {product?.premium_profile?.website}</a>
        </div>
        }
